...
//            Orbwalking.OnNonKillableMinion += Event_OnNonKillableMinion;
        ...
        static void menuload() {
            ...
            MiscM.Add(new MenuKeyBind("fleeKey", "Flee Toggle",Keys.T,KeyBindType.Press));
        ...
        static void Game_OnUpdate(EventArgs args) {
            ...
            if (onupdatetimers != null) {
                if ((Game.ClockTime - onupdatetimers) > (1 / kalm.GetValue<MenuSlider>("onupdateT").Value)) {
                    onupdatetimers = null;
                } else { return; }
            }

            if (kalm.GetValue<MenuBool>("killsteal").Value) {
                Killsteal();
            }
            if (Player.IsRecalling()) { return; }

            if (kalm.GetValue<MenuBool>("harassActive").Value) {
                harass();
            }
            if (kalm.GetValue<MenuBool>("jungleActive").Value) {
                Jungleclear();
            }
            if (kalm.GetValue<MenuBool>("autoW").Value) {
                AutoW();
            }
            if (kalm.GetValue<MenuKeyBind>("fleeKey").Active) {
                ShowjumpsandFlee();
            }
        ...
        static void harass() {
            var lqmana = kalm.GetValue<MenuSlider>("harassmanaminQ").Value;
            var lemana = kalm.GetValue<MenuSlider>("harassmanaminE").Value;
            var minmana = lqmana;
            ...
            if (kalm.GetValue<MenuBool>("harassQ").Value && mymana > lqmana && Q.IsReady(1) && !Player.IsDashing()) {
            ...
            if (!kalm.GetValue<MenuBool>("harassuseE").Value) { return; }
            if (mymana < lemana || !E.IsReady()) { return; }

            if (kalm.GetValue<MenuSlider>("harassE").Value >= 1 && kalm.GetValue<MenuBool>("harassEoutOfRange").Value) {
                var minhealth = kalm.GetValue<MenuSlider>("harassEminhealth").Value;
...
//                var Minions = MinionManager.GetMinions(Player.ServerPosition, R.Range, MinionTypes.All, MinionTeam.NotAlly).FindAll(x => (x.Health + (x.HPRegenRate / 2)) < GetEDamage(x) && ECanCast(x) && x.HealthPercent >= minhealth);
                ...
                if (Minions != null && Minions.Count() >= kalm.GetValue<MenuSlider>("harassE").Value) {
        ...
        static HitChance hitchanceQ() {
            switch (kalm.GetValue<MenuSlider>("harassQchance").Value) {
                case 1:
...
/*        static List<Obj_AI_Base> Q_GetCollisionMinions(Obj_AI_Hero source, Vector3 targetposition) {
        ...
        static void Jungleclear() {
            var mymana = Manapercent;
            if (mymana < kalm.GetValue<MenuSlider>("jungleclearmana").Value) { return; }

            //baron / dragon
            if (kalm.GetValue<MenuBool>("bardragsteal").Value) {
            ...
            if (kalm.GetValue<MenuBool>("jungleclearQ").Value) {
                if (Q.CanCast(jungleinside)) { Q.Cast(jungleinside); }
            }
            if (kalm.GetValue<MenuBool>("jungleclearE").Value) {
        ...
        static void laneclear() {
            ...
            var lqmana = kalm.GetValue<MenuSlider>("laneclearmanaminQ").Value;
            var lemana = kalm.GetValue<MenuSlider>("laneclearmanaminE").Value;
            var minmana = lqmana;
            var mymana = Manapercent;
            if (lemana < minmana) { minmana = lemana; }
            ...
            if (kalm.GetValue<MenuBool>("laneclearQ").Value && Q.IsReady() && mymana >= lqmana && !Player.IsDashing()) {
...
//                if (minionsQ != null) {Q.Cast(minionsQ);}
            ...
            if (kalm.GetValue<MenuBool>("laneclearE").Value && E.IsReady() && mymana >= lemana && !Player.IsDashing()) {
                var minhealth = kalm.GetValue<MenuSlider>("laneclearEminhealth").Value;
                var minionsE = Minions.Where(x => (x.Health + (x.HPRegenRate / 2)) < GetEDamage(x) && ECanCast(x) && x.HealthPercent >= minhealth);
                double laneclearE = kalm.GetValue<MenuSlider>("laneclearEcast").Value;
                double incrementE = kalm.GetValue<MenuSlider>("laneclearEcastincr").Value;
                if (minionsE != null && minionsE.Count() >= Math.Round(laneclearE + (Player.Level * (incrementE / 10)))) {
                    ECast();
                } else if (minionsE != null && kalm.GetValue<MenuBool>("laneclearbigminionsE").Value) { //kill siege/super minions when it can E
        ...
        static void Event_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args) {
            ...
            if (!kalm.GetValue<MenuBool>("savesoulbound").Value) { return; }
            if (sender.IsMe) {
            ...
            if (sender.IsEnemy && sender.Distance(soulmate.ServerPosition) < 1500f) {
                if (soulmate.HealthPercent <= kalm.GetValue<MenuSlider>("savesoulboundat").Value) {
                    R.Cast();
                }
            }
        }
        ...
        static void Event_OnNonKillableMinion(AttackableUnit minion) {
            ...
            if (!kalm.GetValue<MenuBool>("laneclearE").Value || !E.IsReady() || !ECanCast(minionX)) { return; }
            if (Manapercent < kalm.GetValue<MenuSlider>("laneclearmanaminE").Value) { return; }
            if (kalm.GetValue<MenuBool>("laneclearlasthit").Value) {
                var minhealth = kalm.GetValue<MenuSlider>("laneclearEminhealth").Value;
        ...
        static void Event_OnLevelUp() {
            if (kalm.GetValue<MenuBool>("AutoLevel").Value) {
        ...
        static void Drawing_OnDraw(EventArgs args) {
            if (kalm["Drawings"]["ondrawT"].GetValue<MenuSlider>().Value > 1) {
                DraWing.drawtext("lalalelela", 3, Drawing.Width * 0.45f, Drawing.Height * 0.90f, Color.PapayaWhip, "Levelling up Spells");
            }
            if (Player.IsDead) { return; }
            var curposition = Player.Position;
            var ondrawmenutimer = (1 / kalm.GetValue<MenuSlider>("ondrawT").Value);
